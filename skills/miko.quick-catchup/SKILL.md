...
- 実装マッピングを更新する
- **横断概念の書き込み時検査（lint）** — 更新後、`.miko/guides/business_rules_guide.md` の「書き込み時の検査（lint）」を実行する（orphan `xc:` 参照・宣言 ID の重複・宣言行とタグの書式）。問題があれば修正し、修正できないものはステップ 8 末尾の確認で主さまに報告する
