...
`.miko/guides/harae_guide.md` の「機械照合: 横断概念の網羅チェック」に従い、メインセッションで実施する。指摘があれば検証軸「不完全性」としてステップ 4 の統合結果に加える。
...
ステップ 2 の機械照合の指摘とサブエージェントの結果を統合し、重複を排除して `harae.md` を生成する。
