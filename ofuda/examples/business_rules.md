...
このケイパビリティに通底する価値判断・指針。ビジネスルールの背景にある考え方を明文化する。
...
- **ORD-01** [制約] バックオーダー（在庫なし受注）は受け付けない。注文確定時点で在庫が引き当て済みでなければならない。
  ...
  <!-- 紙運用: 台帳でも「在庫がなければ受注しない」は成立 → ドメインビジネスルール -->
- **ORD-02** [制約] 出荷済みの注文はユーザー操作ではキャンセルできない。返品プロセスへ誘導する。
  <!-- 判定: 代替案「出荷後もキャンセル可能にし、配送を止める」はありえる → ビジネスルール -->
  <!-- 紙運用: 出荷後の処理は紙台帳でも変わる → ドメインビジネスルール -->
- **ORD-03** [導出] キャンセル猶予期間は注文確定から24時間と定義する。
  <!-- 判定: 48時間でも即時不可でもよい。24時間はビジネスが決めた値 → ビジネスルール -->
  <!-- 紙運用: 時間の閾値は紙でも成立 → ドメインビジネスルール -->
- **ORD-04** [制約] 猶予期間経過後のキャンセルは管理者操作でのみ可能とする。
  <!-- 判定: 代替案「猶予期間後は一切キャンセル不可」「カスタマーサポート経由で受付」→ ビジネスルール -->
  <!-- 紙運用: 権限による分岐は紙でも成立 → ドメインビジネスルール -->
- **ORD-05** [制約][xc:order_management/DEFAULT] 支払い不履行ユーザーが同時に持てる未決済の注文は 1 件までとする。決済済みの注文には影響しない。
  <!-- 判定: 代替案「注文自体を禁止する」「通常ユーザーと同じ制限のまま」→ ビジネスルール -->
  <!-- 紙運用: 台帳の未決済件数で判定できる → ドメインビジネスルール -->
  <!-- 横断カテゴリの効果を担うルールの例。タグの xc: ID が宣言（DEFAULT カテゴリ）への機械可読なリンク（import に相当）。
       効果ルールの置き場は概念のカテゴリではなく、そのルールが属する自分のドメインのカテゴリでよい。
       他ケイパビリティ（予約・レンタル等）が効果を持つ場合も、まったく同じ形で自分の BR のルールにこのタグを付ける（サンプル用コメント） -->
...
- ORD-05 → `Ec::Order#confirmable?` / 未決済注文数の判定
...
- **PAY-01** [導出] 確定から24時間以内に決済が完了しない注文はキャンセル対象と定義する。
  ...
  <!-- 紙運用: 「未払いなら 24 時間でキャンセル」は紙台帳でも成立（実施を日次バッチでやるか毎朝の人手確認でやるかは実装の話） → ドメインビジネスルール -->
...
#### 支払い不履行ユーザー（DEFAULT）[横断] <!-- cross-category -->

未決済の自動キャンセルを繰り返したユーザー。
各ケイパビリティは、与信が発生する操作（後払い・予約・取り置き等）の制限を表明すること。

<!-- 横断カテゴリのオーナー側の例。[横断] マークでこのカテゴリは public モジュールになり、
     ID は xc:order_management/DEFAULT。効果を持つケイパビリティのルールがタグで参照する
     （オーナー自身の効果ルールも同様。ORD-05 参照）。見出し行の cross-category マーカーが
     grep の対象。lead 文が宣言文（各ケイパビリティが何を表明すべきかの軸だけを書き、
     他ケイパビリティの操作名は列挙しない）（サンプル用コメント） -->

**ルール:**
- **DEFAULT-01** [導出] 直近90日間で未決済による自動キャンセル（PAY-01）が3回発生したユーザーは「支払い不履行ユーザー」と定義する。
  <!-- 判定: 回数・期間はビジネスが決めた値。「1回で即」「定義しない」もありえた → ビジネスルール -->
  <!-- 紙運用: 台帳の記録から判定できる → ドメインビジネスルール -->

<details><summary>実装マッピング</summary>

- DEFAULT-01 → `Ec::Orders::PaymentDefaultQuery` / 直近90日の自動キャンセル集計

</details>

<!-- last: DEFAULT-01 -->

> **サンプル解説（実際の business_rules.md には書かない）:** この概念が「横断」なのは、注文管理の外の操作にも効くからである。効果を持つ**他ケイパビリティ側**の書き方は、例えば `miko/reservation/business_rules.md`（予約管理）では次のようになる — 自分のドメインのカテゴリに自分のルールとして書き、タグで参照する:
>
> ```markdown
...
- **RETRY-01** [制約] 決済プロバイダの応答失敗は注文確定をロールバックしない。非同期リトライで整合性を回復する。
  ...
  <!-- 紙運用: 「非同期リトライ」「整合性回復」は紙運用に対応する概念がない → システムビジネスルール -->
