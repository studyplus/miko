...
**プラグイン** と **スクリプト** の2つのインストール方法がございます。どちらの場合も、インストール後にプロジェクトのセットアップを行います。
...
スキルは `miko:` 名前空間付き（例: `/miko:setup`）で呼び出せます。**本 README はプラグイン版の表記 `/miko:***` で統一しております。** スクリプト版をお使いの場合は `/miko-***`（コロンをハイフンに）とお読み替えくださいませ。

出力言語（日本語 / English）は、初回の `/miko:setup` 実行時に選択いただきます。選択した言語は `.miko/config` に保存され、miko の対話と生成ドキュメントがその言語に統一されます。
...
**プラグイン版:** Claude Code 内で以下を実行くださいませ。プラグインの更新後、`/miko:upgrade` が `miko/` 配下のドキュメント（BR・HLD 等）に必要なマイグレーションと `.miko/` の資材更新を実行いたします。
...
### カスタムスキルのプロテクト（スクリプト版のみ）

`miko-*` という名前でご自身のスキルを作成している場合、アップグレード時に削除されないよう **プロテクト** できます。`.miko/protected_skills` にスキル名を1行ずつ記述くださいませ（スクリプト版のスキルは `.claude/skills/miko-<名前>/` として配置されます）。

プラグイン版では、プロジェクトの `.claude/skills/` はアップグレードの影響を受けないため、プロテクトは不要でございます。
...
| `/miko:upgrade` | プラグイン版の更新後処理。`.miko/` の資材更新と `miko/` 配下ドキュメントのマイグレーションを実行 |
