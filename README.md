...
| `/miko.refine-br <capability> [フェーズ]` | 既存 business_rules.md を判断を変えずに現在の基準へ整える。フォーマット最新化 → 語彙・粒度の言い換え → 分割統合の提案 → 横断チェックをフェーズごとに確認しながら進める |
...
/miko.refine-br <capability>
  ...
  → フェーズC: 分割・統合の提案（ルールID が動くもの。対応表は主さまがお決めになる）
  → フェーズD: ケイパビリティ横断チェック（境界・重複・同音異義語・帰属。検出のみ）
  → 適用記録: 本文やルールIDが動いた場合、適用済みのリファクタリング記録を proposal に残す
     （本文から落とした経緯・却下した代替案の移設先にもなる）

/miko.refine-br <capability> C
  → 途中のフェーズから再開（フェーズごとの確認で打ち切ってよい）
```
