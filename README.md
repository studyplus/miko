...
- **speckit はお調べの道具** — speckit の成果物は使い捨て。Claude に深くコードを読ませるための仕掛けとして使います
...
- [speckit](https://github.com/github/spec-kit) スキルがインストール済みであること（miko は speckit を拡張して動作いたします）
...
| `/miko.quick_impl <proposal \| capability \| 変更指示>` | 小規模な変更を speckit スキップで直接実装。proposal なしの変更指示（リファクタリング等）も受け付ける。BR 本文の変更を伴う場合は proposal が必要 |
...
/miko.split_proposal <proposal>
  → 変更が大きい場合、親（umbrella）+ サブにフェーズ分割（任意）
```
...
/miko.quick_impl <capability>
  → speckit をスキップして直接実装（単一の意図・既存構造の範囲内の変更が対象）
...
`/miko.speckit.implement` は機能ごとに以下のセルフレビューを自動実行いたします:

1. **セルフレビュー** -- 責務の配置、命名、フレームワーク規約のチェック
2. **/simplify** -- コードの重複・品質・効率のチェック

最終フェーズでは機能横断のリファクタリングを実行いたします。
