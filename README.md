...
| `/miko.refine-br <capability> [proposal]` | 既存 business_rules.md を判断を変えずに現在の基準へ整えるリファクタリング提案を作る。フォーマット最新化 → 語彙・粒度の言い換え → 分割統合 → 横断チェックをフェーズごとに確認しながら進め、変更は proposal にまとめる。proposal を渡すと適用する |
...
### 既存 BR の書き直し（リファクタリング）

既存の `business_rules.md` は、明示的な移行依頼がない限り現在の基準に合わせ込まれません。合わせ込むときは `/miko.refine-br` をお使いくださいませ。**ドメインの判断は変えず**、書きっぷりと配置だけを整えます。

**変更はすべて proposal になります。** 起草中は business_rules.md を一切書き換えません。フェーズごとの確認はセッション上に消えるものであり、記録として残るのは proposal だからです。

```
/miko.refine-br <capability>
  → フェーズA: フォーマット最新化（ポリシーセクション新設・2.1/2.2分割・採番コメント）
  → フェーズB: 安全な言い換え（コード語彙の除去・経緯の除去・ADR の HLD 移設）
  → フェーズC: 分割・統合（ルールID が動くもの。対応表は主さまがお決めになる）
  → フェーズD: ケイパビリティ横断チェック（境界・重複・同音異義語・帰属。検出のみ）
  → リファクタリング提案を1本お納めする（本文から落とした経緯の移設先にもなる）

# Optional
/miko.harae <capability> <proposal>
  → 分割・統合を含む場合は検証をお勧めいたします

/miko.refine-br <capability> <proposal>
  → 提案を business_rules.md に適用し、harae.md の参照を付け替える
```

どのフェーズの確認でも打ち切れます。打ち切っても business_rules.md は無傷ですので、次回はまた最初からお始めくださいませ（フェーズ単位の再開機構は持ちません）。

判断そのものを変える必要が見つかった場合（実装とルールの乖離、ルールごとの削除等）は `/miko.propose` にご誘導いたします。
