...
miko can be installed either as a **plugin** or via the **install script**. Either way, set up your project afterwards:
...
```
/plugin marketplace add studyplus/miko
/plugin install miko@miko
```

Plugin skills are invoked with the `miko:` namespace (e.g. `/miko:setup`). **This README uses the plugin form `/miko:***` throughout.** For the script installation, read it as `/miko-***` (colon becomes a hyphen).

You choose the output language (日本語 / English) the first time you run `/miko:setup`. Your choice is saved to `.miko/config`, and miko's conversation and generated documents are unified in that language.
...
**Plugin installation:** run the following inside Claude Code. After the plugin itself is updated, `/miko:upgrade` runs any migrations needed for the documents under `miko/` (business rules, high-level designs, etc.) and refreshes the `.miko/` resources.

```
/plugin update miko@miko
/miko:upgrade
```

**Script installation:** run the following at your project root.
...
### Protecting custom skills (script installation only)

If you have created your own skills named `miko-*`, you can **protect** them from being removed during upgrades. List the skill names in `.miko/protected_skills`, one per line (script-installed skills live at `.claude/skills/miko-<name>/`):
...
With the plugin installation, your project's `.claude/skills/` is untouched by upgrades, so no protection is needed.
...
| `/miko:upgrade` | Post-update step for the plugin installation. Refreshes the `.miko/` resources and runs migrations for the documents under `miko/` |
