...
1ルール＝1つの判断。本文はそれを言い切る最小限の長さにする。原則1〜2文（例外・補足で別の判断を1つ足すときに限り2文目まで）。3文以上、または本文がおおむね150字を超えたら黄信号で、複数判断の同居・経緯の漏れ・仕様の混入を疑い、**分解 / 経緯を proposal へ移動 / 仕様を機能仕様・HLD へ除外** のいずれかで対処する。150字はハード上限ではなく再点検の合図であり、意味の切り詰めや不自然な分割のための数値ではない。
