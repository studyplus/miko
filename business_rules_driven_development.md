...
- **ビジネスポリシー（ビジネス方針）** — ケイパビリティを貫く価値判断・指針。客観的には判定できない
- **ビジネスルール** — ポリシーから導かれる具体的な決まり。任意のケースで客観的に判定できる
...
ビジネスルールには **業務寄りのもの（ドメインルール）** と **システム寄りのもの（システムルール）** がある。

- **ドメインルール** — システムが存在しない（紙台帳と人手だけで運用する）世界でも意味として成立するルール
- **システムルール** — 非同期処理・冪等性・整合性回復など、システムの存在を前提とする概念に依存するルール

**判定: 「紙運用」テスト** — 紙運用で意味として成立するか？

既存のビジネスルールはコードから抽出されることが多く、表現がコードの語彙に引きずられる。「自動キャンセル」「即時反映」のような語は紙運用なら「日次でキャンセル」「翌朝確認」と置き換え可能で、意味としてはドメインルール。一方「非同期リトライで整合性回復」は紙運用に対応する概念がなく、システムルール。**コードの表現を一旦剥がして意味で判定する**。

1 つのルールにドメイン判断とシステム判断が混ざることは避ける。混在していたら分解する。

カタログ上は `### 2.1 ドメインルール` と `### 2.2 システムルール` の 2 セクションに分けて配置し、各カテゴリ（ORD、PAY 等）はどちらか一方に置く。
...
- **ビジネスポリシー**: `POL-{連番}` 形式。ケイパビリティを貫く価値判断。3〜5 個程度を目安に少数に保つ
- **2.1 ドメインルール / 2.2 システムルール**: ルールを「紙運用」テストで振り分けて配置する。カテゴリ（ORD、PAY 等）はどちらか一方に置く
