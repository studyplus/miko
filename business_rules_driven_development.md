...
この構造は新規ケイパビリティ向けであり、既存の `business_rules.md` には遡及適用しない。既存 BR を現在の基準（構造・語彙規律・粒度）へ合わせ込むのは `/miko.refine-br` の役目で、明示的に依頼したときだけ動く。合わせ込みは **判断を変えない範囲** に限られ、判断が動くものは proposal に回る。
